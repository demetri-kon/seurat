...
#' and the default is RNA or Spatial
...
GetResidual <- function(
  ...
  assay <- assay %||% DefaultAssay(object = object)
  if (is.null(umi.assay)) {
    default.assays <- c("RNA", "Spatial")
    umi.assay <- default.assays[default.assays %in% Assays(object)]
    if (length(umi.assay) > 1) {
      umi.assay <- umi.assay[1]
